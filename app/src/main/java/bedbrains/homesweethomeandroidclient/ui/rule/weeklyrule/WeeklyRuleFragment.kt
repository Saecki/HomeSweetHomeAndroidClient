...
class WeeklyRuleFragment : Fragment() {
    ...
    private fun displayTimeSpans(timeSpans: List<WeeklyTimeSpan>, preview: Boolean = false) {
        this.timeSpans.forEach { timeLayout.removeView(it) }
        timeSpans
            .filter { weeklyRuleViewModel.previewTimeSpan?.uid != it.uid }
            .forEach { this.timeSpans.addAll(displayTimeSpan(it, preview)) }
    }

    private fun displayTimeSpan(t: WeeklyTimeSpan, preview: Boolean = false): List<View> {
        ...
        for (i in t.start.localizedDay until endDay + 1) {
            val day = i % 7
            val card = createTimeSpan(day, t, preview)
    ...
    private fun createTimeSpan(day: Int, timeSpan: WeeklyTimeSpan, preview: Boolean = false): View {
        val card = MaterialCardView(requireContext())

        card.background = ContextCompat.getDrawable(requireContext(), R.drawable.time_span_background)
        card.id = View.generateViewId()
        card.tag = timeSpan.uid
        card.isSelected = preview
    ...
    private fun onTimeSpanEdited(event: WeeklyTimeSpanEditEvent) {
        ...
        when (event.action) {
            WeeklyTimeSpanEditEvent.Action.FINISHED -> {
                ...
                previewTimeSpans.forEach { it.isSelected = false }
            ...
            WeeklyTimeSpanEditEvent.Action.PREVIEW -> {
                ...
                previewTimeSpans.forEach { timeLayout.removeView(it) }
                previewTimeSpans.clear()
                previewTimeSpans.addAll(displayTimeSpan(event.timeSpan, true))
